...
use FindBin;
use lib $FindBin::Bin."/BLUEPRINT-dcc-loading-scripts/model/schema+agents/lib";
...
sub rsIdRemapper($$\@) {
	...
	if($rsidCount > 0) {
		$LOG->info("dbSNP coordinates and MAF mapping from rsId ($rsidCount) using vcfagents");
		...
		# This is needed due the unwanted behavior of vcfagents, which always creates a log file
		my $curdir = getcwd();
		my $ND = File::Temp->newdir(TMPDIR => 1);
		chdir($ND->dirname);
		my $mappedRsidCount = 0;
		if(open(my $VCF,'-|','vcfagents','--gzvcf',$localDbSnpVCFFile,'--snps',$rsidFilename,'--recode','--recode-INFO','CAF','--stdout')) {
			while(my $line=<$VCF>) {
			...
			$LOG->logcroak("[ERROR] Unable to run vcfagents on $rsidFilename. Reason: ".$!);
		}
...
sub rsIdRemapOne($$$) {
	...
	unless(exists($rsIdMapping{$snp_id})) {
		...
		my $ND = File::Temp->newdir(TMPDIR => 1);
		chdir($ND->dirname);
		if(open(my $VCF,'-|','vcfagents','--gzvcf',$localDbSnpVCFFile,'--snp',$snp_id,'--recode','--recode-INFO','CAF','--stdout')) {
			while(my $line=<$VCF>) {
				next  if(substr($line,0,1) eq '#');
				chomp($line);
			...
			$LOG->logcroak("[ERROR] Unable to run vcfagents on $localDbSnpVCFFile. Reason: ".$!);
		}
		# Going back to the original directory
		chdir($curdir);
	}
